...
#
    ...
    # CUDA_VISIBLE_DEVICES=$GPU python3 src/data_gen_pde.py num_workers=10  IC_per_param=$ICs_equation data.param_range_gamma=0  data.param_range_shift=1.15 data.types=${dataset}  size=5120 directory=$directory file_name=${file_name}

    file_name=indomain0_eval
    rm $directory/$dataset/${dataset}_${ICs_equation}_${file_name}.prefix
    rm $directory/$dataset/${dataset}_${ICs_equation}_${file_name}_data.h5
    CUDA_VISIBLE_DEVICES=$GPU python3 src/data_gen_pde.py num_workers=10  IC_per_param=$ICs_equation data.param_range_gamma=0.2  data.param_range_shift=1.15 data.types=${dataset}  size=5120 directory=$directory file_name=${file_name}

#    file_name=0to3
#    rm $directory/$dataset/${dataset}_${ICs_equation}_${file_name}.prefix
#    rm $directory/$dataset/${dataset}_${ICs_equation}_${file_name}_data.h5
#    CUDA_VISIBLE_DEVICES=$GPU python3 src/data_gen_pde.py num_workers=10  IC_per_param=$ICs_equation data.param_range_gamma=0.1  data.t_range=[0,3] data.t_num=96 data.types=${dataset}  size=5120 directory=$directory file_name=${file_name}
